...
import { postProjetos } from "../../services/projetosServices"
...
const NovoProjeto = () => {
    ...
    const aoSalvarProjeto = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    return (
        <FormNovoProjeto onSubmit={(e: React.FormEvent<HTMLFormElement>) => aoSalvarProjeto(e)}>
            ...
            <TextAreaAdmin value={descricaoBackEnd} onChange={(e) => setDescricaoBackEnd(e.target.value)}>Descrição BackEnd</TextAreaAdmin>
            <TextAreaAdmin value={descricaoFrontEnd} onChange={(e) => setDescricaoFrontEnd(e.target.value)}>Descrição FrontEnd</TextAreaAdmin>
            <ContainerStacks>
            ...
            <ListaStacks>
                    {stacks.length > 0 && stacks.map((stack, index) => (
                        <ItemStack key={index}>
                            {stack}
                            <p key={`P${index}`} onClick={() => aoDeletarStack(index)}>X</p>
                        </ItemStack>
                    ))}
            </ListaStacks>
            <ContainerBotoes>
